...
class TestJson(unittest.TestCase):
    def test_json(self):
        ...
        latest_dir = "./latest_results/ddpg"
...
def create_json(b_csvs, g_csvs, trails, seeds):
    ...
    for trail in range(trails):
        seed = seeds[trail]
        trail_seed = "trail%d_seed_%d" % (trail + 1, seed)
        result[trail_seed] = {}
        df_g = pd.read_csv(g_csvs[trail])
        df_b = pd.read_csv(b_csvs[trail])
        result[trail_seed]["garage"] = json.loads(df_g.to_json())
        result[trail_seed]["baselines"] = json.loads(df_b.to_json())
    return result
